...
OWNERS      = [6059673725, 8699517479]
DB_NAME     = 'bot_database.db'
PAYOUT_CHAT_ID  = -1003550407610
PAYOUT_TOPIC_ID = 26
PAYOUT_PERCENTAGE = 0.7
CHANNEL_USERNAME  = 'perehodnikAsterix'
PEREHOD_CHAT_ID   = -1003980785442
PEREHOD_LINK      = 'https://t.me/+rC3g5UaRXkEyMmQx'
...
DIRECTIONS = [
    ('escort',   'Эскорт', 5776023601941582822),
    ('trade',    'Трейд',  5231200819986047254),
    ('shantazh', 'Шантаж', 5188217332748527444),
    ('nft',      'NFT',    6037175527846975726),
]
DIRECTION_NAMES = {k: name for k, name, _ in DIRECTIONS}
_DIR_FALLBACK = {'escort': '🪭', 'trade': '📊', 'shantazh': '🕵️', 'nft': '🎁'}

DIRECTION_DISPLAY = {
    k: f'{e(eid, _DIR_FALLBACK[k])} {name}' for k, name, eid in DIRECTIONS
}
...
async def init_db():
    async with db_connect() as conn:
        ...
        await conn.execute('''CREATE TABLE IF NOT EXISTS surveys (
            ...
            answer3 TEXT, status TEXT DEFAULT 'pending', direction TEXT)''')
        try:
            await conn.execute('ALTER TABLE surveys ADD COLUMN direction TEXT')
        except Exception:
            pass
...
async def check_subscribed(user_id: int) -> bool:
    try:
        member = await bot.get_chat_member(PEREHOD_CHAT_ID, user_id)
...
async def smart_edit(msg, text: str, reply_markup=None):
    ...
    is_media = bool(msg.photo or msg.video or msg.animation or msg.document or
                    msg.audio or msg.voice or msg.sticker)
    try:
        if is_media:
        ...
        await bot.send_message(msg.chat.id, text, reply_markup=reply_markup)
...
def main_menu(is_admin_user=False) -> InlineKeyboardMarkup:
    team_link = get_custom('team_link') or 'https://t.me/+bE2rthMh3Ng5NGUy'
    redirect_link = get_custom('redirect_link') or PEREHOD_LINK
    rows = [
...
def sub_check_kb() -> InlineKeyboardMarkup:
    return kb(
        [InlineKeyboardButton.model_construct(text='Перейти в переходник', url=PEREHOD_LINK, icon_custom_emoji_id='5424818078833715060')],
...
def direction_kb(user_id: int) -> InlineKeyboardMarkup:
    rows = [[btn(name, f'dir_{key}_{user_id}', eid)] for key, name, eid in DIRECTIONS]
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def format_profile(user_id, nickname, username, role, profits, join_date) -> str:
    ...
    if user_id in OWNERS:
        role_e, role_t = E['crown'], 'Владелец'
    elif role == 'admin':
        role_e, role_t = E['crown'], 'Админ'
    else:
        role_e, role_t = E['bolt2'], 'Воркер'
    return (
        f'<b>{E["user3"]} {html.escape(str(nickname))}</b> (@{html.escape(str(username))})\n\n'
        f'<blockquote>'
...
async def cmd_start(msg: Message, state: FSMContext):
    ...
    welcome_text = custom_text if custom_text else (
        f'<b>{E["sparkle"]} Добро пожаловать в Onyx Pay!</b>\n\n'
...
async def survey_q3(msg: Message, state: FSMContext):
    data     = await state.get_data()
    answers  = data.get('answers', {})
    answers['q3'] = html.escape(msg.text or '')
    await state.update_data(answers=answers)
    await state.set_state(Survey.direction)
    await msg.answer(
        f'<b>{E["check"]} Принято.</b>\n\n'
        f'<b>{E["question"]} Выберите направление, в котором хотите работать:</b>',
        reply_markup=direction_kb(msg.from_user.id))


@router.callback_query(F.data.startswith('dir_'))
async def cb_survey_direction(cb: CallbackQuery, state: FSMContext):
    parts = cb.data.split('_')
    key, target_id = parts[1], int(parts[2])
    if cb.from_user.id != target_id:
        await cb.answer('❌ Не для вас')
        return
    if await state.get_state() != Survey.direction.state:
        await cb.answer('👋 Напишите /start')
        return
    ...
    data     = await state.get_data()
    answers  = data.get('answers', {})
    direction_name = DIRECTION_NAMES.get(key, key)
    direction_disp = DIRECTION_DISPLAY.get(key, direction_name)
    await state.clear()

    async with db_connect() as conn:
        await conn.execute(
            'INSERT OR REPLACE INTO surveys (user_id,answer1,answer2,answer3,status,direction) VALUES (?,?,?,?,"pending",?)',
            (user_id, answers.get('q1'), answers.get('q2'), answers.get('q3'), key))
        await conn.execute(
            'INSERT OR IGNORE INTO users (user_id,username,nickname,role,join_date,approved) VALUES (?,?,?,"worker",?,0)',
        ...
        await conn.commit()
    await smart_edit(cb.message,
        f'<b>{E["mail"]} Спасибо!</b>\nВаша анкета ({direction_disp}) отправлена на рассмотрение администратора.')
    await cb.answer()
    ...
    for adm in admins:
        try:
            await bot.send_message(adm,
                ...
                f'<b>{E["bolt2"]} Направление:</b> {direction_disp}\n\n'
                f'<blockquote>'
...
async def cb_approve_survey(cb: CallbackQuery):
    ...
    async with db_connect() as conn:
        ...
        await conn.execute('UPDATE users SET approved=1, join_date=? WHERE user_id=?',
...
async def render_top(period: str = 'all') -> str:
    ...
    async with db_connect() as conn:
        if period == 'all':
            async with conn.execute(
                'SELECT user_id,nickname,total_profits,username,role FROM users WHERE approved=1 ORDER BY total_profits DESC LIMIT 10') as c:
                users = await c.fetchall()
            async with conn.execute('SELECT SUM(total_profits) FROM users WHERE approved=1') as c:
                total = (await c.fetchone())[0] or 0
        else:
            async with conn.execute(f'''
                SELECT u.user_id, u.nickname, COALESCE(SUM(p.profit_amount * p.user_percentage), 0) as earned,
                       u.username, u.role
                FROM users u
    ...
    for i, (uid, nick, profit, uname, role) in enumerate(users, 1):
        if i <= 3: medal = medals[i-1]
        elif i <= 10: medal = nums[i-4]
        else: medal = f'{i}.'
        label = html.escape(nick or (f'@{uname}' if uname else f'Воркер {i}'))
        link = f'https://t.me/{uname}' if uname else f'tg://user?id={uid}'
        name = f'<a href="{link}">{label}</a>'
...
async def cb_top_period(cb: CallbackQuery):
    ...
    if period not in TOP_PERIODS:
        period = 'all'
    try:
        await smart_edit(cb.message, await render_top(period), reply_markup=top_period_kb(period))
    except Exception:
        pass
    await cb.answer()
...
async def cb_top(cb: CallbackQuery):
    if not await require_approved(cb.from_user.id, cb):
        return
    await smart_edit(cb.message, await render_top('all'), reply_markup=top_period_kb('all'))
    await cb.answer()
...
async def cb_info(cb: CallbackQuery):
    ...
    team_link = get_custom('team_link') or 'https://t.me/+bE2rthMh3Ng5NGUy'
    team_name = get_custom('team_name') or 'Onyx Pay'
    if custom_info:
        text = custom_info
    else:
        text = (
            f'<b>{E["info"]} {team_name}</b>\n\n'
            f'{E["crown"]} <b>Владелец:</b> @nftobi + @cutkeep\n'
            f'{E["hat"]} <b>Кодер:</b> @cutkeep'
        )
    await smart_edit(cb.message, 
        text,
        reply_markup=kb(
...
async def cb_my_payouts(cb: CallbackQuery):
    ...
    try:
        await smart_edit(cb.message, text, reply_markup=my_payouts_kb(payouts, page, total, per_page))
    except Exception:
        pass
    await cb.answer()
...
async def payout_deal_code(msg: Message, state: FSMContext):
    ...
    if not msg.text or not msg.text.startswith('#'):
        ...
        return
    payout_temp[user_id]['deal_code'] = html.escape(msg.text)
    await state.set_state(Payout.media)
    media_text = (
...
async def cb_media_done(cb: CallbackQuery, state: FSMContext):
    ...
    try:
        await smart_edit(cb.message, nft_text)
    except Exception:
        try: await cb.message.delete()
        except: pass
...
async def payout_gift_link(msg: Message, state: FSMContext):
    ...
    if not valid_links:
        ...
        return
    payout_temp[user_id]['gift_link'] = html.escape('\n'.join(valid_links))
...
async def payout_mentor_username(msg: Message, state: FSMContext):
    ...
    if not mentor:
        return
    payout_temp[user_id]['mentor_username'] = html.escape(mentor)
    await state.clear()
...
async def submit_payout(msg: Message, user_id: int, from_user=None):
    ...
    if mentor_username:
        mentor_pct = float(get_custom('mentor_percent') or 0.4)
        pct = round(pct * (1 - mentor_pct), 4)
...
async def admin_enter_reject_reason(msg: Message, state: FSMContext):
    ...
    reason = html.escape((msg.text or '').strip())
...
async def cb_admin_user_payouts(cb: CallbackQuery):
    ...
    nick = html.escape((urow[0] if urow else None) or (urow[1] if urow else str(uid)))
...
async def cb_delete_user_confirm(cb: CallbackQuery):
    ...
    if not row:
        ...
        return
    nick, uname = row
    display = html.escape(nick or (f'@{uname}' if uname else str(uid)))
    await smart_edit(cb.message,
...
async def cmd_check(msg: Message):
    try:
        member = await bot.get_chat_member(PEREHOD_CHAT_ID, bot.id)
    ...
    if subscribed:
        ...
        await msg.answer(
            ...
            f'{E["arrow"]} Добавь бота в переходник ({PEREHOD_LINK}) как администратора.')
